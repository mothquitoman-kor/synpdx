# ---- BIC extractor for SaemixObject -----------------------------------
get_bic_saemix <- function(obj) {
  # try to get BIC from saemix::AIC output
  ...
  if (!inherits(val, "try-error")) {
    ...
    if (!is.null(nms)) {
      ...
      for (nm in cand_names) {
        if (nm %in% nms && is.finite(val[[nm]])) {
          return(unname(val[[nm]]))
        }
      }
  ...
  # fallback: compute BIC from log-likelihood and n,k
  ll <- try(obj@results@ll.is,  silent = TRUE)
  if (!(is.numeric(ll) && length(ll) == 1 && is.finite(ll))) {
    ll <- try(obj@results@ll.lin, silent = TRUE)
  }
  
  if (is.numeric(ll) && length(ll) == 1 && is.finite(ll)) {
    k <- try(length(obj@results@fixed.effects), silent = TRUE)
    if (!is.numeric(k) || !is.finite(k)) k <- 0
    
    n <- try(nrow(obj@data@data), silent = TRUE)
    if (!is.numeric(n) || !is.finite(n)) n <- 0
...
synpdx_model_fit <- function(dat, control, drug_a, drug_b, combo,
  ...
  model_map <- list(
    exp_constA_constB      = exp_constA_constB,
    exp_constA_decayB      = exp_constA_decayB,
    exp_decayA_constB      = exp_decayA_constB,
    exp_decayA_decayB      = exp_decayA_decayB,
    gomp_constA_constB     = gomp_constA_constB,
    gomp_constA_decayB     = gomp_constA_decayB,
    gomp_decayA_constB     = gomp_decayA_constB,
    gomp_decayA_decayB     = gomp_decayA_decayB,
    logistic_constA_constB = logistic_constA_constB,
    logistic_constA_decayB = logistic_constA_decayB,
    logistic_decayA_constB = logistic_decayA_constB,
    logistic_decayA_decayB = logistic_decayA_decayB
  )
  ...
  # ---------- auto selection over candidate models by BIC ----------
  if (model == "auto") {
    ...
    for (i in seq_along(cand)) {
      ...
      if (inherits(obj, "SaemixObject")) {
        fits[[i]] <- obj
        bics[i]   <- get_bic_saemix(obj)
      }
    }
    if (!any(is.finite(bics))) stop("All candidate models failed to fit.")
    best_idx <- which.min(bics)
    saem_obj <- fits[[best_idx]]
    picked   <- cand[best_idx]
  } else {
    if (!model %in% names(model_map)) {
      stop("Unknown model: ", model, ". Must be one of: ", paste(names(model_map), collapse = ", "))
    }
